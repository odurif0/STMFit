...
def _annotate_plot(source: Path, destination: Path, lines: list[str], confidence: str, guard_vs_final: str) -> None:
    """Draw a minimal scientific-figure banner above the plot.

    Two-column layout — big *N final* label on the left, metadata on the
    right, all clipped to the image width so nothing overflows.
    ...
    banner_h = 145
    annotated = Image.new("RGB", (width, height + banner_h), (255, 255, 255))
    annotated.paste(image, (0, banner_h))
    ...
    n_y = (banner_h - n_ink_h) // 2 - n_bbox[1]
    draw.text((pad_x, n_y), n_text, fill=(30, 30, 30), font=n_font)

    # --- Vertical separator ---
    sep_x = pad_x + n_w + 34
    draw.line((sep_x, 18, sep_x, banner_h - 18), fill=(210, 210, 210), width=2)
    ...
    draw.text((meta_text_x, meta_y + 36), _fit_text(draw, lines[3], meta_font, meta_w), fill=(100, 100, 100), font=meta_font)

    # --- Bottom accent line ---
    draw.line((0, banner_h - 2, width, banner_h - 2), fill=accent, width=3)
