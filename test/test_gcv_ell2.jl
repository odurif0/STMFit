...
ccfg_circ = GaussianFit2D.ChainSweepConfig(
    ...
    chain_tilted_baseline=true, chain_circular_sigmas=true,  # ← CIRCULAR
    cv_method="gcv", selection_criterion="gcv",
)

results_circ, _, ctx_circ = GaussianFit2D.chain_gaussian_sweep(img, pcfg, ccfg_circ)

# Refine to elliptical
ccfg_ell = deepcopy(ccfg_circ); ccfg_ell.chain_circular_sigmas = false
results_ell = _refine_circ_to_ell(results_circ, img, pcfg, ccfg_ell, ctx_circ)

println("N   GCV(circ)   GCV(ell)    RSS(circ)   RSS(ell)    BIC(circ)   BIC(ell)")
println(repeat("-", 85))
for n in sort(unique([r.n for r in results_circ if r.success]))
    ...
    println(rpad(string(n),4),
            ...
            rc !== nothing ? rpad(@sprintf("%.1f", rc.bic), 12) : rpad("---",12),
            re !== nothing ? rpad(@sprintf("%.1f", re.bic), 12) : rpad("---",12))
end
